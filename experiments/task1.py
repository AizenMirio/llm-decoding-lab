...
from experiments.generate_constrained import ConstrainedTextGenerator
from experiments.task0 import get_dataloader, set_seed, clean_text
...
if __name__ == "__main__":
    ...
    parser.add_argument("--word-list","-wl", type=str, required=True, help="Path to a tab-separated word-list file with one line per benchmark example.")
    ...
    parser.add_argument("--debug","-db", action="store_true", help="Print input, reference, and generated text.")
    ...
    with open(args.word_list, 'r') as f:
        word_lists = [line.strip() for line in f if line.strip()]
    ...
    if len(word_lists) < total:
        raise ValueError(
            f"Expected at least {total} word-list lines, but found {len(word_lists)} in {args.word_list}."
        )
